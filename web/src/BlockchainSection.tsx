...
export default function BlockchainSection({ networks, onNetworksChanged }: BlockchainSectionProps) {
	// FIX: Guard against null being passed down
			...
			{showForm && (
				<form onSubmit={submit} className="inline-form">
					...
					<div className="field">
						...
						<input id="bc-rpc1" className="input" value={rpc1} onChange={e => setRpc1(e.target.value)} placeholder="https://..." required />
					</div>
					<div className="field">
						<label className="label" htmlFor="bc-rpc2">RPC Endpoint 2 (optional)</label>
						<input id="bc-rpc2" className="input" value={rpc2} onChange={e => setRpc2(e.target.value)} placeholder="https://..." />
					</div>
					<div className="field">
						...
						<tbody>
							{safeNetworks.map(n => (
								<tr key={n.id}>
									...
									<td className="mono muted">{n.rpc_endpoint_1.replace(/^https:\/\//, '').slice(0, 24)}…</td>
